...
Updated 21 September 2026 on branch `impl/evidence-platform-phase0`.
...
| --- | --- | --- | --- | --- | --- |
| R01 | One clean, reviewable commit is the release candidate; all application, migration, contract, lock and deployment files it needs are tracked | Commit SHA, clean `git status`, reviewed diff and release notes | Required | Required | **PASS** — candidate `3478c9891c79d01ef3d08535d810aa92830f5ebd` is clean, pushed, is the head of pull request #1, and every imported source path is tracked; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| R02 | A versioned, reproducible artifact is built from R01 without relying on workstation state | Artifact identifier and SHA-256; clean-clone build log | Required | Required | **PASS** — exact-candidate artifact `trendsell-3478c9891c79.tar.gz` was built twice byte-identically in a clean clone; SHA-256 `d2117dd6c78c5ce99d4e968d63f6594c5f25e69b7f9b3c0344afdabbe2d7ea1d` was re-verified independently of CI and its embedded manifest pins the candidate commit; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| R03 | Remote CI passes on R01: SQLite and PostgreSQL backend suites, frontend unit/type/build, browser suite, clean-clone build, dependency scan and secret/history scan | Links to all successful workflow jobs for the SHA | Required | Required | **PASS** — all seven jobs in [run 35554333213](https://github.com/NdumLab/trendsell-app/actions/runs/35554333213) passed for the exact candidate SHA; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
...
| F02 | Every workspace-data read, write, export and audit route enforces tenant and role boundaries; denial leaves the other tenant's state unchanged | Route inventory and shared-database isolation tests on R01 | Required | Required | **PASS** — exact `3478c98` candidate passes 68 tenancy, authorization and audit regressions locally against PostgreSQL and the full suite against PostgreSQL in remote CI; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| F03 | Retried and concurrent account, job, review, decision and watch operations converge without duplicate effects, lost updates or unexpected 500s | PostgreSQL contention/idempotency results on R01 | Required | Required | **PASS** — exact `3478c98` candidate passes the 8 PostgreSQL contention and idempotency cases locally and in remote CI; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| F04 | Saved decisions, evidence snapshots, formula/threshold versions and exports remain immutable, internally consistent and replayable | Contract, replay and historical-lineage tests on R01 | Required | Required | **PASS** — exact `3478c98` candidate passes 199 contract, replay and historical-lineage tests locally against PostgreSQL and in remote CI; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| F05 | Fresh databases and a sanitized representative pre-release schema both migrate to head; physical-schema readiness succeeds afterward | Migration logs, row/backfill counts and `/api/ready` result | Required | Required | **PASS** — carried to `3478c98` on byte-identical code trees from the `e954830` exercise, which migrated a fresh PostgreSQL 16.4 database, preserved 900 representative synthetic records, completed a no-op candidate upgrade and became ready after two production-mode two-worker starts; see the [F05 closure](releases/2026-09-20-f05-closure.md) and the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
| F06 | Invalid, oversized, unauthorized, timed-out and unavailable-dependency cases fail safely without private-data leakage or false success | Negative API/browser tests and staged fault exercise | Required | Required | **PASS** — the exact `3478c98` artifact, run in production mode with two workers from its own runtime lock, returned 422/401/413/413/202 with a real 15.060 s SMTP timeout through a socket that accepted and never greeted; the disposable address, password, rate-key secret and metrics token appear zero times in captured logs; see the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md#f06--staged-negative-and-dependency-fault-exercise) |
...
| S08 | Runtime and development dependencies contain no unaccepted known high/critical vulnerability; reachable history and release artifacts contain no credential; licenses are compatible | CI scan links, artifact/history scan and dependency/license inventory | Required | Required | **PASS** — `pip-audit --strict` is clean on both locks locally and in CI, credential scans pass on tracked files, reachable history and the unpacked artifact, and the 181-package license policy passes; exact inventory and dispositions are in [DEPENDENCY_LICENSES.md](DEPENDENCY_LICENSES.md) and the [confirmation record](releases/2026-09-21-candidate-3478c98-confirmations.md) |
...
| T02 | The live-data pilot states exact source/market/field coverage, separates catalog identity, demand indicators, local listing candidates, observed sales and attribution, and explains that outputs are indicators/scenarios rather than forecasts or professional approval | Pilot agreement, in-product copy and participant comprehension check | Required | Required | **BLOCKED** — proposed scope is in `releases/2026-09-17-live-data-pilot.md`; owners and users have not approved/exercised it |
| T03 | Before the controlled pilot, permitted collectors produce real dated catalog, demand and available local-market observations with lineage, snapshots, bounded retries and expiry; Data Health reports typed collection/storage/API telemetry, while a real browser session separately proves display. FX may remain an explicit user assumption in the minimum pilot. A representative end-to-end result is independently reconciled in production | Source approvals, collector/failure/expiry tests and real production browser reconciliation | Required | Required | **BLOCKED** — adapters exist only in the undeployed candidate; production has zero live observations |
