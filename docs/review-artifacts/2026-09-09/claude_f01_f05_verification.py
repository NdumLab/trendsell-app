"""F01-F05 verification: the independent reproductions, rerun against the corrections.

This is `independent_recheck.py` with ONE behavioural change, described below. The
reviewer's original is unmodified next to this file and remains the record of what was
run at 71cd584.

Why an adaptation was needed: the original synchronizes its two redemptions inside
`hash_password`, which requires BOTH requests to reach hashing. That is only possible
while the defect exists. With the token claimed atomically under a row lock, the second
request blocks on the lock and is refused *before* it hashes, so the two-party barrier
can never be satisfied and the original aborts with `BrokenBarrierError` instead of
reporting a result. Here the requests are synchronized on entry instead, and the hash
barrier tolerates a lone arrival -- same overlap, no assumption that the bug is present.

Disposable test data only. Same disposable-schema handling as the original.
...
def check(url, postgres=False):
    ...
    with TestClient(app) as client:
        ...
        if postgres:
            ...
            entry = Barrier(2, timeout=15)
            original_hash = passwords.hash_password

            def synchronized_hash(value):
                # Adapted: once the race is closed only ONE request reaches hashing (the
                # other blocks on the row lock and is refused before it hashes), so a
                # strict two-party barrier can no longer be satisfied. Waiting with a
                # tolerated timeout keeps the original intent -- hold the winner inside
                # the transaction long enough for the loser to overlap it -- without
                # presuming the defect is still present.
                try:
                    barrier.wait()
                except Exception:
                    pass
            ...
            def redeem(value):
                entry.wait()          # both requests start together
