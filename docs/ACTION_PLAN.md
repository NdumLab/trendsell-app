...
Created: 8 September 2026. Last corrected: 10 September 2026. Status: **in progress**.
...
| P03 account recovery | Partial — delivery blocked | **Implemented:** browser flows for password reset/change, active sessions, purpose-bound email verification and one-person-workspace deletion; hashed short-lived tokens; non-enumerating responses; address/account limits; transactional invalidation and PostgreSQL single-winner coverage. A reset no longer claims the email was verified, diagnostic `log` never claims delivery, production rejects both `sink` and `log`, and the runbook's operator issuance takes the same account row lock as the API. **Remaining:** choose a provider/sending identity, implement and test its transport, and monitor deliverability. Until then production keeps `MAIL_TRANSPORT` empty and uses the documented operator procedure |
...
| Gate A: trustworthy manual workflow | ~~Done — 2026-09-08~~ **In review — 2026-09-10** | R01–R11 and F01–F05 are corrected with regressions, followed by the route/role/isolation and deployment/recovery follow-up. Local verification is current: 505 backend tests passed on SQLite with 16 PostgreSQL-only skips, all 521 passed on PostgreSQL, plus 141 frontend tests and 46 browser tests. Gate A stays in review until an independent reviewer signs off the exact release commit and remote CI runs. |
| Gate B: recoverable platform | Partial — 2026-09-10 | Clean deployment/adoption and a disposable current-head PostgreSQL dump/restore/replay are verified; permissions, recovery mechanics and privacy behavior are documented. **Outstanding:** real mail delivery or explicit operator acceptance, production restore drill, scheduled/offsite backups, agreed RPO/RTO and retention, installed log verification and operational ownership. |
