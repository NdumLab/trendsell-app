...
Phase 1 is **partially** implemented. Account flows, owner-managed membership, a TLS SMTP transport, bounded audit retention, and an encrypted offsite-backup path now ship. Outstanding within it: configuring and proving a real mail provider, the production backup/restore drill and recovery objectives (P06), the pilot privacy notice and qualified reviewer ownership (P05), and alert forwarding to an on-call destination (P07). The table below is authoritative where this paragraph and it disagree. Selected Phase 2/3 items — manual evidence, the evidence-quality method, the compliance-review workflow and server-computed gates — are implemented against self-reported evidence only. **No live collector is connected**, so all production evidence remains self-reported and capped at 60 confidence, below the 70 a GO requires: a real end-to-end GO is not yet demonstrable.
...
| P01 migrations and deployment config | Done | `backend/migrations/` 0001→0006, `python -m app.migrate {check,upgrade,stamp,check --against}`, `deploy/` templates with placeholders only, `docs/RUNBOOK.md`. **Reopened and re-fixed (R08, R09):** the adoption guide checked a deliberately-behind baseline against today's models and expected stamping 0001 to leave it current; readiness trusted the revision label over a schema missing `audit_events`. The documented sequence is now executed verbatim by `test_migrations.py` |
| P02 password hashing | Done | `backend/app/passwords.py`; self-describing scrypt at an OWASP-listed set; legacy hashes verify and are rehashed on a correct sign-in, now including passwords with leading/trailing whitespace (R02). A locked-out pilot user has the explicit operator recovery path under P03; self-service still requires real mail delivery |
| P03 account recovery | Partial — delivery activation blocked | **Implemented:** browser flows for password reset/change, active sessions, purpose-bound email verification and workspace deletion; hashed short-lived tokens; non-enumerating responses; address/account limits; transactional invalidation and PostgreSQL single-winner coverage; browser links keep tokens in URL fragments. A TLS-enforced SMTP transport supports authenticated STARTTLS or implicit TLS, while diagnostic `sink`/`log` remain rejected in production. **Remaining:** choose and configure the provider/sending identity, SPF/DKIM/DMARC, end-to-end delivery/bounce tests and monitoring. Until then production keeps `MAIL_TRANSPORT` empty and uses the documented operator procedure |
...
| P05 permissions and audit | Partial | `backend/app/permissions.py` matrix; owner-managed, email-bound invitations; role changes and immediate member suspension; requesters cannot decide their own compliance review; audit carries actor/workspace/request/target detail and expires under `AUDIT_RETENTION_DAYS`; behavior is documented in `PRIVACY_AND_PERMISSIONS.md`. **Remaining:** publish the matching pilot notice, approve the production retention value, and name qualified independent reviewers—the software separates people but cannot establish credentials or independence |
| P06 backup and restore | Partial | Scripts and `verify_restore.py` present; every published dump has a checked SHA-256 sidecar; optional private-S3 upload requests server-side encryption and fails the job if either object is not copied; drill run against a disposable PostgreSQL instance. **Reopened and re-fixed (R06):** the verifier replayed assessments without stored evidence and grouped workspaces by display name. A hardened daily service/timer is present but deliberately not installed. **Not done:** production drill, bucket/IAM/lifecycle approval, enabling/monitoring the schedule, retention decision and agreed recovery objectives |
| P07 logs, request ids, metrics | Partial | Structured application logs omit raw paths, queries, addresses, bodies and credentials; network rate keys are hashed; production disables Uvicorn's raw access line and gives nginx a reduced access format; `X-Request-ID` reaches audit rows; metrics use a separate operator token; audit retention is enforced. **Not done:** verify the installed proxy/service with disposable markers, choose log retention/access ownership, and forward alerts to an on-call destination |
| E01 source feasibility matrix | Partial — access and human decisions blocked | First-party capability, geography, cost/quota and public-use terms are recorded in `releases/2026-09-17-live-data-feasibility.md` for catalog/history, search, Nigeria marketplace, FX, freight, regulations, ads/creators and sales. The smallest pilot is DataForSEO catalog/discovery/search plus Bright Data Jumia; Open Exchange Rates is an optional second increment. Published DataForSEO terms/product material establish own-product use, and Open Exchange Rates documents caching. Bright Data still requires its compliance process to accept the declared `jumia.com.ng` target and intended normalized use; internal approvals remain pending. Amazon Creators is rejected under its standard affiliate/cache terms |
| E02–E05 collectors and snapshots | Partial — implemented, disabled and never collected live | Server-side DataForSEO catalog/discovery/Trends, Bright Data Jumia and Open Exchange Rates adapters include bounded retries, normalization, candidate matching, content-addressed dedup, versioned normalized snapshots, enforced expiry and source telemetry. UI evidence and typed Data Health distinguish configuration, collection, stored observations and API availability; browser display has separate fixture-labelled coverage. Tests use fixtures only. No approved account/credential or real response exists and production runs an older artifact, so this is not a live connection |
...
| E07 evidence-quality method | Implemented for the pilot | `evidence-quality/1.1.0` with advertising, creator, search and marketplace demand categories, per-component explanations, a 90-day window, and a cap of 60 on self-reported evidence; `contracts/evidence_cases.json` holds both implementations together. Data Health now reports configured, connected, degraded or unavailable source state with workspace-scoped attempt/freshness telemetry |
| N02 classification model | Done | Candidate codes stay candidates; every rule carries publisher, link and effective date |
| N03 compliance-review workflow | Done | Request → decide → supersede, behind `compliance.review`; expiry enforced; a request must be decided by a different workspace reviewer. **Reopened and re-fixed (R03, R04):** an approval citing only expired sources, with no classification and no requirements, cleared the gate; and a rejection produced WATCH because the decision received only `compliance_resolved=False`. Approvals must now rest on a source in force and name what they approved; a rejection forces NO-GO without the user restating it |
...
4. Saved economics inputs, calculations and versions remain immutable. New inputs, evidence, classification, or formulas create a new version. Provider evidence payloads are the narrow exception: expiry removes them from current records and replaces historical copies with explicit retention tombstones.
...
| Gate A: trustworthy manual workflow | ~~Done — 2026-09-08~~ **In review — 2026-09-10** | R01–R11 and F01–F05 are corrected with regressions, followed by the route/role/isolation and deployment/recovery follow-up. Current-head local verification: 532 backend tests passed on SQLite with 16 PostgreSQL-only skips, all 548 passed on PostgreSQL, plus 141 frontend tests and 47 browser tests. Gate A stays in review until an independent reviewer signs off the exact release commit and remote CI runs. |
