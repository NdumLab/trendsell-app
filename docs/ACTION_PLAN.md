...
| E01 source feasibility matrix | Partial — provider approval blocked | The Amazon Creators API protocol and available fields are documented, but its standard Associates terms describe referral applications and short-lived caching (one hour for offers and BrowseNodeInfo; one day for other product content). TrendSell's internal research and durable evidence snapshots therefore remain unapproved; do not enable the adapter without explicit Amazon terms covering both use and retention |
| E02–E05 collectors and snapshots | Partial — disabled | A server-side GetItems adapter, workspace source health, response hashes, versioned parsing and controlled-response tests exist. It is off by default and is not a live collector: no credential or provider approval has been supplied, and manual capture (E06) remains the current production path |
