...
| P03 account recovery | Partial — delivery blocked | **Implemented:** password reset with hashed, single-use, purpose-bound, 30-minute tokens; non-enumerating request responses; rate limits per address and per account; authenticated password change that revokes other sessions; session listing and revocation. `backend/app/mail.py` provides the transport seam and the local sink; `tests/redesign/test_account_recovery.py` exercises recovery end to end through the sink, reading the token out of the message rather than the database. Migrations `0004_account_recovery` and `0005_email_verification`. **Also implemented (F02, and the review's instruction to separate local work from the provider decision):** verified email ownership end to end against the sink — a 24-hour, single-use, purpose-bound token issued at registration and on request, `email_verified` on the account, and purposes that are not interchangeable; a password change now invalidates every outstanding recovery credential; a reset token is claimed atomically under a row lock, so two concurrent redemptions produce exactly one success (F01); and account/workspace deletion behind the password and a typed phrase, refusing a shared workspace rather than half-deleting it. **Blocked on the provider decision:** actual delivery only — with no provider, `MAIL_TRANSPORT` is empty, nothing is sent, and a locked-out user still needs an operator; that operator procedure is still unwritten. **Delivery is not configuration-only:** `mail.build()` accepts empty, `sink` and `log`, so a provider means a new transport class and a settings branch, as `docs/RUNBOOK.md` describes. **Browser flows now shipped:** password reset from the sign-in dialog (request and redeem), password change, active-session listing and revocation, email verification and workspace deletion all have screens; `e2e/account-security.spec.ts` drives each of them against the local sink, reading the token out of the message rather than the database. **Remaining:** delivery only, plus the operator procedure for a locked-out user, now written up in `docs/RUNBOOK.md` |
...
| Gate A: trustworthy manual workflow | ~~Done — 2026-09-08~~ **In review — 2026-09-09** | Claimed done on 8 September; the independent review the same day declined to sign off, because an evidence-bearing draft disagreed with its own saved assessment (R01) and the full export omitted evidence and compliance reviews (R05) — two of Gate A's own acceptance criteria. R01–R11 were corrected; the 9 September re-review then found R01, R03 and R07 still open in narrower cases plus two new recovery defects (F01–F05), and those are now corrected and verified in turn. The gate stays **in review** until an independent review signs off — no re-review has signed off yet. Current: **504 backend tests collected — 488 executed on SQLite (16 skipped, requiring PostgreSQL) and 504 executed on PostgreSQL 16.4**, 141 frontend tests, 45 browser tests, TypeScript, production build. Dependency scans were not rerun for F01–F05: the lockfiles are unchanged. Each reviewed defect has a regression that fails against the pre-fix code, confirmed by reverting each fix individually. |
