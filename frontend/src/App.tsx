...
function PasswordRecovery({onDone}:{onDone:()=>void}){
 ...
 const {data:config}=useQuery({queryKey:['config'],queryFn:()=>api<PublicConfig>('/config'),retry:false});
...
function AuthDialog(){
 ...
 const {data:config}=useQuery({queryKey:['config'],queryFn:()=>api<PublicConfig>('/config'),retry:false,enabled:authOpen});
 ...
 {config?.invitation_only&&!register&&<Notice>This is a free, invitation-only controlled pilot. Sign in with your invited address, or ask a workspace owner for an invitation.</Notice>}
...
function fragmentToken(hash:string){return new URLSearchParams(hash.slice(1)).get('token')||'';}
function PasswordResetLink(){
 const w=useWorkspace();const location=useLocation();const navigate=useNavigate();
 const [busy,setBusy]=useState(false);const [error,setError]=useState('');
 const token=fragmentToken(location.hash);
 return <div className="token-action-page"><section className="panel token-action-card"><ShieldCheck size={28}/><h1>Reset your TrendSell password</h1><p>Choose a new password. Redeeming this single-use link ends every session on the account, including any an intruder may hold.</p>
  <form className="stack" onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');const data=new FormData(event.currentTarget);try{await post('/auth/recovery/reset',{token:String(data.get('token')).trim(),password:data.get('password')});await w.refresh();navigate('/',{replace:true});w.setAuthOpen(true);toast.success('Password reset. Sign in with your new password.');}catch(problem){setError((problem as Error).message);}finally{setBusy(false);}}}>
   <label>Reset token<input name="token" required minLength={20} maxLength={200} autoComplete="off" defaultValue={token} placeholder="Paste the token from your message"/></label>
   <label>New password<input name="password" type="password" required minLength={12} maxLength={128} autoComplete="new-password" placeholder="At least 12 characters"/></label>
   {error&&<div role="alert" className="form-error">{error}</div>}
   <button className="button primary full" disabled={busy}>{busy?'Resetting…':'Set a new password'}<ArrowRight size={16}/></button>
  </form><Link className="text-button centered" to="/">Back to sign in</Link></section></div>;
}
function EmailVerificationLink(){
 const w=useWorkspace();const location=useLocation();const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [verified,setVerified]=useState(false);
 const token=fragmentToken(location.hash);
 return <div className="token-action-page"><section className="panel token-action-card"><ShieldCheck size={28}/><h1>Verify your TrendSell email</h1><p>This single-use link proves control of the address that received it. It does not expose the token to HTTP access logs.</p>
  {verified?<><Notice>Email address verified.</Notice><Link className="button primary full" to={w.user?'/settings':'/'}>{w.user?'Open account settings':'Continue to sign in'}<ArrowRight size={16}/></Link></>:
  <form className="stack" onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');const data=new FormData(event.currentTarget);try{await post('/auth/email-verification/confirm',{token:String(data.get('token')).trim()});await w.refresh();setVerified(true);toast.success('Email address verified');}catch(problem){setError((problem as Error).message);}finally{setBusy(false);}}}>
   <label>Verification token<input name="token" required minLength={20} maxLength={200} autoComplete="off" defaultValue={token} placeholder="Paste the token from your message"/></label>
   {error&&<div role="alert" className="form-error">{error}</div>}
   <button className="button primary full" disabled={busy}>{busy?'Verifying…':'Verify this address'}<ArrowRight size={16}/></button>
  </form>}
 </section></div>;
}
function InvitationAcceptance(){
 ...
 const [busy,setBusy]=useState(false);const [error,setError]=useState('');
 const token=fragmentToken(location.hash);
 return <div className="token-action-page"><section className="panel token-action-card"><ShieldCheck size={28}/><h1>Join your TrendSell workspace</h1><p>Choose your name and password. The invitation is bound to the email address that received it and can be used once.</p>
  {w.user&&<Notice kind="amber">Accepting this invitation will sign this browser into the invited account.</Notice>}
  <form className="stack" onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');const data=new FormData(event.currentTarget);try{await post<User>('/auth/invitations/accept',{token:String(data.get('token')).trim(),name:data.get('name'),password:data.get('password')});await w.refresh();navigate('/',{replace:true});toast.success('Invitation accepted');}catch(problem){setError((problem as Error).message);}finally{setBusy(false);}}}>
   <label>Invitation token<input name="token" required minLength={20} maxLength={200} autoComplete="off" defaultValue={token} placeholder="Paste the token from your invitation"/></label>
   <label>Your name<input name="name" required minLength={1} maxLength={80} autoComplete="name"/></label>
   <label>Password<input name="password" type="password" required minLength={12} maxLength={128} autoComplete="new-password" placeholder="At least 12 characters"/></label>
   {error&&<div role="alert" className="form-error">{error}</div>}
   <button className="button primary full" disabled={busy}>{busy?'Joining…':'Join workspace'}<ArrowRight size={16}/></button>
  </form></section></div>;
}
function Shell(){
 ...
 {w.demo?<div className="mode-banner"><FlaskConical size={14}/><strong>DEMO WORKSPACE</strong><span>Synthetic examples. No live market data or real investment recommendations.</span><button onClick={w.exitDemo}>Exit demo<X size={13}/></button></div>:<div className="pilot-banner"><span className="small-dot"/>{config?.invitation_only?'FREE · INVITATION-ONLY PILOT':'PILOT WORKSPACE'}<span className="pilot-banner-detail">{config?.invitation_only?'Controlled cohort. No billing or public sign-up.':'A focused corridor. A clearer decision.'}</span><button onClick={()=>void w.enterDemo()}>Explore demo<ArrowUpRight size={13}/></button></div>}
 <main id="main-content" tabIndex={-1}>{w.error&&<div className="connection-error"><Notice kind="amber">{w.error} <button className="text-button" onClick={()=>void w.refresh()}>Retry connection</button></Notice></div>}<React.Suspense fallback={<Loading/>}><Routes><Route path="/" element={<Today/>}/><Route path="/reset-password" element={<PasswordResetLink/>}/><Route path="/verify-email" element={<EmailVerificationLink/>}/><Route path="/accept-invitation" element={<InvitationAcceptance/>}/><Route path="/discover" element={<Discover/>}/><Route path="/xray" element={<Xray/>}/><Route path="/products/:id" element={<ProductView/>}/><Route path="/market-gaps" element={<MarketGaps/>}/><Route path="/decisions" element={<DecisionRoom/>}/><Route path="/watchtower" element={<Watchtower/>}/><Route path="/suppliers" element={<Suppliers/>}/><Route path="/data-health" element={<DataHealth/>}/><Route path="/settings" element={<Settings/>}/><Route path="/products" element={<Navigate to="/discover" replace/>}/><Route path="/research" element={<Navigate to="/xray" replace/>}/><Route path="/watchlist" element={<Navigate to="/watchtower" replace/>}/><Route path="*" element={<div className="not-found"><h1>Let’s get you back on track.</h1><p>This page has moved in the new TrendSell workspace.</p><Link to="/" className="button primary">Back to Today<ArrowRight size={16}/></Link></div>}/></Routes></React.Suspense></main><footer className="app-footer"><span><ShieldCheck size={13}/>Built on evidence. Made for better decisions.</span><span>TrendSell · Free invitation-only pilot</span></footer></div>
